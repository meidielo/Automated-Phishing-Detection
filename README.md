...
`scripts/llm_provider_eval.py` benchmarks configured LLM providers directly on `SAFE`, `VERIFY`, and `DO_NOT_PAY` labels so subscription tiers can be chosen from accuracy, completion rate, latency, and estimated token cost instead of provider branding. It defaults to PII-free rows only and writes ignored JSON/Markdown reports under `data/payment_scam_dataset/reports/`. Use the built-in `--run-live` guard intentionally because it sends the selected redacted/public/synthetic sample text to external LLM APIs.
...
4. **NLP intent classification**: Best results require an LLM API key. For production cost control, start with `LLM_PROVIDER=deepseek` and `DEEPSEEK_API_KEY`; the current PII-free payment-decision pilot favored `deepseek-v4-flash` on accuracy, latency, and cost. Use Anthropic Claude or Kimi only as explicit quality fallbacks after `scripts/llm_provider_eval.py` shows a real gain on your redacted dataset. Moonshot/Kimi and generic OpenAI-compatible Chat Completions providers are also supported. Falls back to a sklearn TF-IDF classifier with reduced accuracy (~70% vs ~92% with LLM).
