...
Optional: DeepSeek is the default cost-first LLM for Starter/Pro/Business NLP intent classification. Set `LLM_PROVIDER=deepseek` with `DEEPSEEK_API_KEY`, then run `scripts/llm_provider_eval.py` on redacted samples before any Enterprise override. Anthropic, Gemini, Kimi, OpenAI, and generic OpenAI-compatible providers are supported for benchmarking, but should only replace DeepSeek when evals show enough quality or scale benefit to justify the extra cost. ANY.RUN/Joe Sandbox keys unlock additional sandbox providers.
...
4. **NLP intent classification**: Best results require an LLM API key. For production cost control, use `LLM_PROVIDER=deepseek`/`DEEPSEEK_API_KEY` for Starter, Pro, and Business. The current PII-free payment-decision pilot favors `deepseek-v4-flash` on accuracy, latency, and cost. Use Anthropic Claude, Kimi, Gemini Pro, GPT-5.5, or other higher-cost models only as Enterprise review candidates after `scripts/llm_provider_eval.py` shows a real gain on your redacted dataset. Moonshot/Kimi, Gemini, OpenAI GPT-5.x, and generic OpenAI-compatible Chat Completions providers are supported. Falls back to a sklearn TF-IDF classifier with reduced accuracy (~70% vs ~92% with LLM).
