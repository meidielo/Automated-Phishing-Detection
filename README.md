...
Set `PUBLIC_DEMO_MODE=true` to expose `/demo` as a sample-only public preview. This is intentionally not an auth bypass for the real app: `/analyze`, `/dashboard`, `/monitor`, `/accounts`, live upload analysis, mailbox monitoring, feedback learning, and account management still require the analyst token. The public root `/` redirects to `/product` so visitors do not land on an admin login.
