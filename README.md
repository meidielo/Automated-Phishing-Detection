...
`scripts/llm_provider_eval.py` benchmarks configured LLM providers directly on `SAFE`, `VERIFY`, and `DO_NOT_PAY` labels so subscription tiers can be chosen from accuracy, completion rate, latency, and estimated token cost instead of provider branding. It supports DeepSeek, Moonshot/Kimi, Gemini, Anthropic, OpenAI GPT-5.x, and generic OpenAI-compatible endpoints. It defaults to PII-free rows only and writes ignored JSON/Markdown reports under `data/payment_scam_dataset/reports/`. Use the built-in `--run-live` guard intentionally because it sends the selected redacted/public/synthetic sample text to external LLM APIs.
...
Current LLM provider pilot (2026-05-03, local ignored payment dataset, PII-free test+holdout rows, 5 samples per decision): DeepSeek V4 Flash scored 14/15 with zero API errors at about $0.0009 total estimated cost; Claude Opus 4.7 scored 13/15 at about $0.0744; Kimi K2.6 scored 12/15 at about $0.0084; DeepSeek V4 Pro scored 10/15 at about $0.0029. Gemini support is wired through `GEMINI_API_KEY` and the same eval slice now covers `gemini-3.1-pro-preview` (10/15, ~$0.0220, 6065 ms median), `gemini-3-flash-preview` (11/15, ~$0.0057, 1668 ms median), and `gemini-3.1-flash-lite-preview` (9/15, ~$0.0026, 979 ms median). OpenAI GPT-5.5 support is wired through `OPENAI_API_KEY`; on the same slice, `gpt-5.4-mini` scored 11/15 at about $0.0066 with 934 ms median latency, while `gpt-5.5` scored 9/15 at about $0.0464 with 2085 ms median latency. This is a pilot, not a production benchmark. Use DeepSeek V4 Flash as the default paid-tier LLM unless a larger real-redacted eval proves Gemini, OpenAI, or another model is materially better; GPT-5.5 should stay as an explicit benchmark/quality-check candidate rather than a default model for this classifier.
...
Optional: DeepSeek and Gemini are the recommended cost-first LLM candidates for NLP intent classification. Set `LLM_PROVIDER=deepseek` with `DEEPSEEK_API_KEY` or `LLM_PROVIDER=gemini` with `GEMINI_API_KEY`, then run `scripts/llm_provider_eval.py` before assigning models to paid tiers. Anthropic can stay as a quality fallback when you explicitly want Claude and accept the higher API cost. OpenAI is supported with `OPENAI_API_KEY`; the OpenAI default is `gpt-5.4-mini`, and `LLM_MODEL=gpt-5.5` should be used only when you want to benchmark the flagship model directly. Moonshot/Kimi and generic OpenAI-compatible providers are also supported. ANY.RUN/Joe Sandbox keys unlock additional sandbox providers.
...
| LLM client | `test_anthropic_client`, `test_openai_compatible_client`, `test_llm_provider_eval`, `test_pipeline_llm_provider` (deterministic JSON calls where providers support them, model version capture, provider wiring, PII-safe provider benchmarking) |
...
4. **NLP intent classification**: Best results require an LLM API key. For production cost control, start with `LLM_PROVIDER=deepseek`/`DEEPSEEK_API_KEY` or `LLM_PROVIDER=gemini`/`GEMINI_API_KEY`; the current PII-free payment-decision pilot favors `deepseek-v4-flash` on accuracy, latency, and cost. Use Anthropic Claude, Kimi, Gemini Pro, GPT-5.5, or other higher-cost models only after `scripts/llm_provider_eval.py` shows a real gain on your redacted dataset. Moonshot/Kimi, Gemini, OpenAI GPT-5.x, and generic OpenAI-compatible Chat Completions providers are supported. Falls back to a sklearn TF-IDF classifier with reduced accuracy (~70% vs ~92% with LLM).
...
- `scripts/docker_deploy.sh` pulls, rebuilds with the current git SHA, reads runtime secrets from `${APP_ENV_FILE:-.env}` in raw Docker Compose env-file mode, requires `CLOUDFLARE_TUNNEL_TOKEN` for production tunnel deploys, and waits for a healthy orchestrator plus a running `cloudflared-tunnel`. `scripts/docker_self_heal.sh` is intended for host cron/systemd to restart containers that Docker marks `unhealthy` but does not restart automatically.
...
| [`docs/security-audit-2026-05-03.md`](docs/security-audit-2026-05-03.md) | Latest MCP, auth, deployment, and dependency audit notes |
