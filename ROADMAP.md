...
| **Cheaper OpenAI-compatible LLM providers** - `LLM_PROVIDER=deepseek`, `moonshot`/`kimi`, `gemini`, `openai`, and `openai_compatible` can route NLP intent classification through provider-specific or generic `LLM_API_KEY` settings; DeepSeek is the default cost-first model for Starter/Pro/Business, while Anthropic, Gemini, Kimi, and OpenAI stay explicit Enterprise benchmark candidates when real-redacted evals justify the extra cost. API Status reports DeepSeek, Moonshot/Kimi, Gemini, and OpenAI key readiness. | `src/analyzers/clients/openai_compatible_client.py`, `src/orchestrator/pipeline.py`, `src/diagnostics/api_checks.py`, `scripts/llm_provider_eval.py` |
...
| **Browser session auth for admin dashboard** - `/admin/login` sets signed session and CSRF cookies; the same `TokenVerifier` accepts bearer or browser session auth. Session lifetime is configurable and `/api/auth/session` exposes browser session state for UI. | `src/security/web_security.py`, `main.py`, `templates/login.html`, `static/shared.js` |
| **Strict admin dashboard CSP and self-hosted chart assets** - dashboard graphs load Chart.js from `/static/vendor/`, dashboard/shared CSS and JS are static files, the global feedback modal stays out of inline page markup, and `/admin/dashboard/` sends a CSP without `'unsafe-inline'`; fallback charts remain for asset/runtime failure. | `main.py`, `src/reporting/dashboard.py`, `templates/dashboard.html`, `static/`, `tests/unit/test_dashboard_session_auth.py` |
...
| **Public sample-only demo mode** - `/demo` can be exposed without analyst login while owner live analysis, global mailbox monitoring, paid API-backed owner checks, feedback learning, and owner account management remain token-protected. | `PUBLIC_DEMO_MODE`, `main.py`, `templates/demo.html`, `static/demo.css` |
...
| **Public PhishAnalyze user routes plus admin split** - `/analyze`, `/dashboard`, and `/monitor` are normal-user SaaS pages with tenant-scoped data; owner tooling moved under `/admin`, and `/admin/api/overview` exposes aggregate privacy-preserving system metrics. | `main.py`, `templates/phish_app.html`, `templates/admin.html`, `static/phish_app.*`, `static/admin.*`, `src/saas/database.py` |
