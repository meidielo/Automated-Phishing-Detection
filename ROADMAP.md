...
| **Cheaper OpenAI-compatible LLM providers** - `LLM_PROVIDER=deepseek`, `moonshot`/`kimi`, `gemini`, `openai`, and `openai_compatible` can route NLP intent classification through provider-specific or generic `LLM_API_KEY` settings; DeepSeek and Gemini are documented as cost-first production candidates while Anthropic remains an explicit quality fallback. API Status reports DeepSeek, Moonshot/Kimi, and Gemini key readiness. | `src/analyzers/clients/openai_compatible_client.py`, `src/orchestrator/pipeline.py`, `src/diagnostics/api_checks.py` |
