...
| **SSRF guard on browser detonation navigation and redirects** | `src/analyzers/url_detonation.py`, `src/extractors/url_extractor.py` | T-TB3, I-TB3: private-network access through redirects, iframes, scripts, images, and helper redirect resolution |
...
| **Autoescaped generated HTML reports** | `src/reporting/dashboard.py`, `src/reporting/report_generator.py` | T-TB6, I-TB6: attacker-controlled subjects, senders, reasoning, and IOC fields cannot execute in generated reports |
...
**Status: MITIGATED.**
**Description:** All state-changing and information-disclosing analyst endpoints in `main.py` (`/api/feedback`, `/api/feedback/retrain`, `/api/accounts/*`, `/api/detonate-url`, `/api/diagnose`, `/api/system-status`, `/admin/api/overview`, `/api/monitor/email/{id}`, `/api/analyze/upload`, `/api/config`, `/api/monitor/alerts`) require `TokenVerifier`. API clients use bearer auth. Browser analysts authenticate through `/admin/login`, which sets a signed HttpOnly session cookie plus a CSRF cookie; state-changing session-cookie requests require a matching `X-CSRF-Token` and same-origin Origin/Referer. Failed analyst token logins and failed SaaS email/password logins are throttled per client address and principal to slow brute-force attempts. The same token (`ANALYST_API_TOKEN` env var) is enforced by the existing `src/feedback/feedback_api.py` router. Normal user routes under `/api/saas/*` use separate signed user sessions, CSRF protection, tenant-scoped `org_id` queries, and plan gates; `/analyze`, `/dashboard`, and `/monitor` are public user shells that show only the signed-in user's workspace data. Stripe subscription webhooks are the deliberate exception to user CSRF because they are server-to-server calls; `/api/stripe/webhook` verifies `Stripe-Signature` with `STRIPE_WEBHOOK_SECRET` before mutating subscription state. `SAAS_PUBLIC_SIGNUP_ENABLED=false` by default, so deployments do not accept visitor email uploads unless the operator deliberately enables signup. `PUBLIC_DEMO_MODE=true` only opens `/demo`, `/api/demo/status`, and `/api/demo/plans`; those routes serve fixed sample content, explicit capability flags, and non-secret plan metadata, not mailbox data, uploaded emails, feedback labels, owner account management, URL detonation, or paid API-backed analysis. Additionally, `run_server()` defaults to `127.0.0.1` and refuses to start on a non-loopback host without `ANALYST_API_TOKEN`.
...
- URL detonation repeats the SSRF guard for the initial URL and for every Playwright navigation, redirect, and subresource request. Blocked redirects and blocked subresources are recorded as risk indicators rather than silently ignored.
- URL redirect resolution repeats the SSRF guard before each HEAD request and redirect hop, including relative redirects resolved against the current URL.
