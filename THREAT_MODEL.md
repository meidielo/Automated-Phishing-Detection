...
The pipeline stores email metadata — sender, recipient, subject, body excerpt, verdict, IOCs — in `data/results.jsonl` and the SQLAlchemy feedback DB. Under the **Australian Privacy Act 1988** and the **EU GDPR**, this is regulated personal information processing. The risk is legal/regulatory, not a confidentiality breach in the security sense, so it's tracked separately from the STRIDE residual risks above.
