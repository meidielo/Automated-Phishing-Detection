...
def test_zoho_password_reset_mailer_refreshes_token_and_sends_mail(monkeypatch):
    ...
    def fake_urlopen(request, timeout):
        ...
        if "/oauth/v2/token?" in request.full_url:
    ...
    token_body = urllib.parse.parse_qs(urllib.parse.urlparse(token_request.full_url).query)
    mail_body = json.loads(mail_request.data.decode("utf-8"))

    assert mailer.enabled is True
    assert token_request.full_url.startswith("https://accounts.zoho.com.au/oauth/v2/token?")
